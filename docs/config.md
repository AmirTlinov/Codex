...
| `auto_attach_agents_context`                     | boolean                                                           | Automatically prepend the `.agents/context` files you selected via `/context` or `config.toml` to every prompt (default `true`). |
