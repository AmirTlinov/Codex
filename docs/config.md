...
| `assistant_language`                             | string                                                            | Force user-facing assistant prose (incl. reasoning shown in the UI and reasoning summaries) to this language; keep code/paths/commands in English. If a provider emits reasoning in a fixed language, Codex may translate it for display (can add an extra model call). `"auto"` disables. |
...
| `memory.max_bytes`                               | number                                                            | Maximum size of the lego memory archive in bytes (default: `52428800`). When exceeded, the archive compacts to a soft cap and may evict non‑pinned blocks. |
