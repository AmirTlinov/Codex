...
| `wrap_break_long_words`                          | boolean                                                           | Controls whether the TUI may break extremely long tokens mid-word when wrapping (default `true`).                          |
| `auto_attach_agents_context`                     | boolean                                                           | Automatically prepend AGENTS.md context to every prompt when available (default `true`). Toggle via `/settings` or by editing `config.toml`. |
| `tui.notifications`                              | boolean \| array<string>                                          | Enable desktop notifications in the TUI (default: false). Toggle it in `/settings` for a simple on/off switch or edit `config.toml` for a custom allowlist. |
