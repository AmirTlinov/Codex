...
use codex_cli::code_nav::run_eval;
    ...
    match subcommand {
        ...
        Some(Subcommand::Navigator(mut navigator_cli)) => {
            ...
            match navigator_cli.command {
                ...
                NavigatorSubcommand::Flow(mut flow_cli) => {
                    prepend_config_flags(
                        &mut flow_cli.config_overrides,
                        navigator_cli.config_overrides.clone(),
                    );
                    run_flow(flow_cli).await?;
                }
                NavigatorSubcommand::Profile(mut profile_cli) => {
                    ...
                    run_profile(profile_cli).await?;
                }
                NavigatorSubcommand::Eval(mut eval_cli) => {
                    prepend_config_flags(
                        &mut eval_cli.config_overrides,
                        navigator_cli.config_overrides.clone(),
                    );
                    run_eval(eval_cli).await?;
                }
            }
        }
