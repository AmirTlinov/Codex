...
- Begin a turn: To send user input, call `turn/start` with the target `threadId` and the user's input. Optional fields let you override model, modelProvider, cwd, sandbox policy, approval policy, approvals reviewer, etc. This immediately returns the new turn object. The app-server emits `turn/started` when that turn actually begins running.
...
- `model/list` — list available models. By default this returns models for the active provider; pass `providers` to request multiple configured providers in one response. Set `includeHidden: true` to include entries with `hidden: true`. Each model also includes `providerId` and `providerName`, plus reasoning effort options, optional legacy `upgrade` model ids, optional `upgradeInfo` metadata (`model`, `upgradeCopy`, `modelLink`, `migrationMarkdown`), and optional `availabilityNux` metadata.
