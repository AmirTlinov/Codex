...
/// Client-supplied configuration for a `codex-shell` MCP tool call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case")]
pub struct CodexShellToolCallParam {
    /// Exact shell command to run through a Codex-owned worker.
    pub command: String,
...
/// Builds a `Tool` definition for the `codex-shell` tool-call.
pub(crate) fn create_tool_for_codex_shell_tool_call_param() -> Tool {
    let schema = SchemaSettings::draft2019_09()
        .with(|s| {
            s.inline_subschemas = true;
            s.option_add_null_type = false;
        ...
        .into_generator()
        .into_root_schema_for::<CodexShellToolCallParam>();

    let input_schema = create_tool_input_schema(schema, "Codex shell tool schema should serialize");

    Tool {
        name: "codex-shell".into(),
        title: Some("Codex Shell".to_string()),
        input_schema,
        output_schema: Some(codex_tool_output_schema()),
        description: Some(
            "Run an exact shell command through a Codex-owned worker session and return the result."
                .into(),
        ),
        annotations: None,
        execution: None,
...
mod tests {
    ...
    #[test]
    fn verify_codex_shell_tool_json_schema() {
        let tool = create_tool_for_codex_shell_tool_call_param();
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Run an exact shell command through a Codex-owned worker session and return the result.",
          "inputSchema": {
            "properties": {
              "approval-policy": {
                "description": "Approval policy for shell commands generated by the model: `untrusted`, `on-failure`, `on-request`, `never`.",
              ...
              "command": {
                "description": "Exact shell command to run through a Codex-owned worker.",
                "type": "string"
              },
              "config": {
                "additionalProperties": true,
