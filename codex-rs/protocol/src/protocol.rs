...
pub enum Op {
    ...
    /// Promote a running foreground shell to a background session managed by the unified exec runtime.
    PromoteShell {
        /// Identifier of the foreground shell call to promote.
        call_id: String,
        /// Optional user-provided description for the promoted shell.
        #[serde(skip_serializing_if = "Option::is_none")]
        description: Option<String>,
    },

    /// Poll an existing background shell for output and lifecycle updates.
    PollBackgroundShell {
        /// Identifier of the background shell to poll.
        shell_id: String,
    },

    /// Restore the workspace to the most recent available snapshot.
    Undo,
...
pub enum EventMsg {
    ...
    /// Notification that a foreground shell was promoted to a background shell.
    ShellPromoted {
        /// Identifier for the original foreground shell call.
        call_id: String,
        /// Identifier assigned to the promoted background shell.
        shell_id: String,
        /// Initial buffered output captured during promotion.
        initial_output: String,
        /// Optional user-provided description associated with this shell.
        #[serde(skip_serializing_if = "Option::is_none")]
        description: Option<String>,
    },

    /// Announcement that an undo task has started.
    UndoStarted(UndoStartedEvent),

    /// Completion status for an undo task.
