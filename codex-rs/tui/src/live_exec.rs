...
impl LiveExecState {
    ...
    pub(crate) fn promote(&mut self, call_id: &str, shell_id: String, initial_output: String) {
        if let Some(entry) = self.entries.get_mut(call_id) {
            entry.promote(shell_id, initial_output);
        }
    }
...
impl LiveExecEntry {
    ...
    fn promote(&mut self, shell_id: String, initial_output: String) {
