...
mod agents_context_manager;
mod agents_context_warning;
mod app;
mod app_backtrack;
mod app_event;
...
pub(crate) fn format_percent(percent: f64) -> String {
    ...
    let mut formatted = format!("{percent:.decimals$}");
...
pub(crate) fn format_token_count<T>(value: T) -> String
where
    T: Into<u64>,
{
    crate::status::format_tokens_compact(value.into())
}
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        match CodexAuth::from_codex_home(&codex_home, config.cli_auth_credentials_store_mode) {
...
fn determine_repo_trust_state(
    ...
    if approval_policy_overide.is_some() || sandbox_mode_override.is_some() {
        ...
        .get_active_project(&config.cwd)
        .map(|project| project.is_trusted())
        .unwrap_or(false)
    {
