...
async fn claude_model_picker_popup_snapshot() {
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("claude_model_picker_popup", popup);
}

#[tokio::test]
async fn mixed_provider_model_picker_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("claude-opus-4-6")).await;
    chat.set_model_provider("claude_cli", codex_core::create_claude_cli_provider());
    ...
    chat.open_all_models_popup(vec![
        crate::model_catalog::ModelCatalogEntry {
            ...
            provider_name: "Claude CLI".to_string(),
            preset: preset(
                "claude-opus-4-6",
                "Claude Opus 4.6",
                "Claude flagship model for the deepest reflective and coding work.",
...
#[tokio::test]
async fn mixed_provider_top_level_model_picker_popup_openai_current_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.set_model_provider(
        "openai",
        codex_core::built_in_model_providers(/*openai_base_url*/ None)["openai"].clone(),
    );
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!(
        "mixed_provider_top_level_model_picker_popup_openai_current",
        popup
    );
}
