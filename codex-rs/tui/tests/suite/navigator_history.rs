...
use codex_tui::history_cell_test_support::navigator_history_lines_for_test;
...
*** End Search"#;
    ...
    let screen = render_history(navigator_history_lines_for_test(
        SEARCH_REQUEST,
        None,
        WIDTH,
    ));
    ...
    assert!(
        screen.contains("Navigator SessionID (rust)"),
...
#[test]
fn json_wrapped_block_is_normalized() {
    const REQUEST: &str = r#"{"action":"*** Begin Search\nquery: App\n*** End Search"}"#;

    let screen = render_history(navigator_history_lines_for_test(REQUEST, None, WIDTH));

    assert!(
        screen.contains("Navigator App"),
        "navigator request not normalized:\n{screen}"
    );
}
...
*** End Search"#;
  ...
  "stats": {
    ...
    assert!(
        screen.contains("core/tests.rs:12"),
...
fn malformed_request_shows_parse_error() {
    ...
    const ERROR_OUTPUT: &str =
        "navigator accepts only *** Begin <Action> blocks; JSON payloads are not supported";
    ...
    assert!(
        screen.contains("navigator block must start"),
