...
static PARSER: Lazy<Mutex<Parser>> = Lazy::new(|| {
    ...
    if let Err(err) = parser.set_language(&tree_sitter_bash::LANGUAGE.into()) {
        panic!("failed to load shell grammar: {err}");
    }
    Mutex::new(parser)
});

pub(crate) fn parse_tree(source: &str) -> Result<tree_sitter::Tree, String> {
    parse_with_cached_parser(&PARSER, "shell", source)
}
...
fn find_candidate(
    ...
    while let Some(current) = stack.pop() {
        if current.kind() == "function_definition"
            && let Some(target) = locate_symbol(path, source, current)
        {
            return Some(target);
        }
...
pub struct ShellSymbolLocator;

impl ShellSymbolLocator {
    pub fn instance() -> &'static dyn SymbolLocator {
        static INSTANCE: ShellSymbolLocator = ShellSymbolLocator;
        &INSTANCE
    }
}

impl SymbolLocator for ShellSymbolLocator {
