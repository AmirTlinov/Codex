...
impl Session {
    ...
    pub(crate) async fn overwrite_history(&self, items: Vec<ResponseItem>) {
    ...
    pub(crate) async fn promote_shell(
        ...
        if let Some(state) = self.services.foreground_shell.get(&call_id).await {
            ...
            let promotion_message = if let Some(desc) = response.description {
                format!(
                    "Foreground shell promoted to background shell {} ({desc}); monitor shell via LiveExec",
                    response.shell_id
                )
            } else {
                format!(
                    "Foreground shell promoted to background shell {}; monitor shell via LiveExec",
                    response.shell_id
                )
            };
    ...
    pub(crate) async fn poll_background_shell(
        ...
        if let Err(err) = manager
            .poll(&shell_id, None, Arc::clone(self), turn_context)
            .await
        {
            self.notify_background_event(
                &sub_id,
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op {
            ...
            Op::PromoteShell {
                ...
                sess.promote_shell(Arc::clone(&turn_context), sub.id, call_id, description)
                    .await;
            }
            Op::PollBackgroundShell { shell_id } => {
                sess.poll_background_shell(Arc::clone(&turn_context), sub.id, shell_id)
                    .await;
            }
            Op::Undo => {
                sess.spawn_task(
