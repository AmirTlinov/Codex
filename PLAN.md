...
## 4. CLI-агент
1. ✅ **Запуск** — если `timeout_ms` превышает 60 с, `shell_run` автоматически стартует процесс в фоне; короткие команды остаются в foreground и могут быть переведены пользователем через `Ctrl+R`.
2. ✅ **Мониторинг** — каждое событие `Promoted`/`Terminated` транслируется в системную реплику вида `[shell-7] …`, поэтому и TUI, и headless клиенты сразу видят, кто перевёл или завершил процесс (включая `ended_by=user`).
3. ✅ **Инструменты** — CLI не просит полные логи (только tail/diagnostic), а `shell_kill` после `AlreadyFinished` лишь обновляет summary; пользователю больше не выдаются прямые `!cmd`.
...
4. **Фаза 4 – Cleanup**
   ...
   - ✅ Backend отвергает `RunUserShellCommand` (показывает warning + system note), а TUI оставляет только подсказку вместо выполнения `!cmd`.
